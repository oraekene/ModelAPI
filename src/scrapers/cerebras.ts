/**
 * Cerebras scraper — fetches model catalog from Cerebras API.
 *
 * Cerebras offers inference with a generous free tier:
 *   - 1,000 requests/day free
 *
 * API docs: https://inference-docs.cerebras.ai/api-reference/models
 */
...
export class CerebrasScraper extends BaseScraper {
  ...
  protected async doHandle(
    ...
    const apiKey = env.CEREBRAS_API_KEY as string | undefined;
    if (!apiKey) throw new Error('CEREBRAS_API_KEY not set');

    const res = await fetch(`${CEREBRAS_API}/models`, {
      headers: { Authorization: `Bearer ${apiKey}` },
    });
    if (!res.ok) throw new Error(`cerebras /models: HTTP ${res.status}`);
    const catalog = (await res.json()) as CerebrasModelsResponse;
    ...
    const rows = models.map((m) => {
      ...
      return stmt.bind(
        ...
        pricePrompt,
        priceCompletion,
        `https://api.cerebras.ai/v1/models/${m.id}`,
        now,
        m.id,
      );
    });
    ...
    await env.DB.prepare(
      ...
      .bind('cerebras-free-tier', 'cerebras', 'Cerebras Free Tier', 'requests_per_day', 1000, 'documented', now)
      .run();

    return { offerings: rows.length, scores: 0, quotas: 1, note: `${rows.length} models from Cerebras API` };
  }
}
