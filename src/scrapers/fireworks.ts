/**
 * Fireworks AI scraper — fetches model catalog from Fireworks API.
 *
 * Fireworks offers inference with signup credits and pay-as-you-go.
 *
 * API docs: https://docs.fireworks.ai/api-reference/list-deployed-models
 */
...
const FIREWORKS_API = 'https://api.fireworks.ai/inference/v1';

export class FireworksScraper extends BaseScraper {
  ...
  protected async doHandle(
    ...
    const apiKey = env.FIREWORKS_API_KEY as string | undefined;
    if (!apiKey) throw new Error('FIREWORKS_API_KEY not set');

    const res = await fetch(`${FIREWORKS_API}/models`, {
      headers: { Authorization: `Bearer ${apiKey}` },
    });
    if (!res.ok) throw new Error(`fireworks /models: HTTP ${res.status}`);
    const catalog = (await res.json()) as FireworksModelsResponse;
    ...
    const rows = models.map((m) => {
      ...
      return stmt.bind(
        ...
        pricePrompt,
        priceCompletion,
        `https://api.fireworks.ai/inference/v1/models/${m.id}`,
        now,
        m.id,
      );
    });
    ...
    await env.DB.prepare(
      ...
      .bind('fireworks-credits', 'fireworks', 'Fireworks Signup Credits', 'credits', 1, 'documented', now)
      .run();

    return { offerings: rows.length, scores: 0, quotas: 1, note: `${rows.length} models from Fireworks API` };
  }
}
