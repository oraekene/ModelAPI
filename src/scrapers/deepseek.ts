/**
 * DeepSeek scraper — fetches model catalog from DeepSeek API.
...
export class DeepSeekScraper extends BaseScraper {
  ...
  protected async doHandle(_msg: ScraperMessage, env: ScraperEnv): Promise<ScraperResult> {
    ...
    const rows = models.map(m => {
      const id = `deepseek/${m.id}`;
      return stmt.bind(id, 'deepseek-api', 'paid', 'api', m.context_length ?? null, 0, 0, 0, 0, 0, `https://platform.deepseek.com/models/${m.id}`, now, m.id);
    });

    if (rows.length > 0) await env.DB.batch(rows);

    await env.DB.prepare(`INSERT INTO quota_pools (pool_id, platform, label, quota_unit, quota_value, confidence, last_verified_at) VALUES (?1,?2,?3,?4,?5,?6,?7) ON CONFLICT(pool_id) DO UPDATE SET quota_value=excluded.quota_value, last_verified_at=excluded.last_verified_at`).bind('deepseek-free-tier', 'deepseek', 'DeepSeek Free Tier', 'credits', 1, 'documented', now).run();

    return { offerings: rows.length, scores: 0, quotas: 1, note: `${rows.length} models from DeepSeek API` };
  }
}
