/**
 * Novita AI scraper — fetches model catalog from Novita API.
 *
 * Novita AI offers inference with signup credits.
 *
 * API docs: https://novita.ai/docs/api-reference/models/list-models
 */
...
const NOVITA_API = 'https://api.novita.ai/v3/openai';

export class NovitaScraper extends BaseScraper {
  readonly id = 'novita';
  ...
  protected async doHandle(
    ...
    const apiKey = env.NOVITA_API_KEY as string | undefined;
    if (!apiKey) throw new Error('NOVITA_API_KEY not set');

    const res = await fetch(`${NOVITA_API}/models`, {
      headers: { Authorization: `Bearer ${apiKey}` },
    });
    if (!res.ok) throw new Error(`novita /models: HTTP ${res.status}`);
    const catalog = (await res.json()) as NovitaModelsResponse;
    ...
    const rows = models.map((m) => {
      ...
      return stmt.bind(
        ...
        pricePrompt,
        priceCompletion,
        `https://api.novita.ai/v3/openai/models/${m.id}`,
        now,
        m.id,
      );
    });
    ...
    await env.DB.prepare(
      ...
      .bind('novita-credits', 'novita', 'Novita Signup Credits', 'credits', 1, 'documented', now)
      .run();

    return { offerings: rows.length, scores: 0, quotas: 1, note: `${rows.length} models from Novita API` };
  }
}
