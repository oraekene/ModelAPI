/**
 * ElevenLabs scraper — fetches model catalog from ElevenLabs API.
...
export class ElevenLabsScraper extends BaseScraper {
  ...
  protected async doHandle(_msg: ScraperMessage, env: ScraperEnv): Promise<ScraperResult> {
    ...
    const rows = models.map(m => stmt.bind(`elevenlabs/${m}`, 'elevenlabs-tool', 'paid', 'tool', null, 0, 0, 0, 0, 0, `https://elevenlabs.io/${m}`, now, m));

    if (rows.length > 0) await env.DB.batch(rows);

    return { offerings: rows.length, scores: 0, quotas: 0, note: `${rows.length} models from ElevenLabs` };
  }
}
