/**
 * Luma Dream Machine scraper — fetches model catalog from Luma API.
...
export class LumaScraper extends BaseScraper {
  ...
  protected async doHandle(_msg: ScraperMessage, env: ScraperEnv): Promise<ScraperResult> {
    const apiKey = env.LUMA_API_KEY as string | undefined;
    if (!apiKey) throw new Error('LUMA_API_KEY not set');

    const now = new Date().toISOString();
    const models = ['dream-machine-1.5', 'dream-machine'];
    ...
    const rows = models.map(m => stmt.bind(`luma/${m}`, 'luma-tool', 'paid', 'tool', null, 1, 0, 0, 0, 0, `https://lumalabs.ai/${m}`, now, m));

    if (rows.length > 0) await env.DB.batch(rows);

    return { offerings: rows.length, scores: 0, quotas: 0, note: `${rows.length} models from Luma Dream Machine` };
  }
}
